...
//댓글에 대한 답글 들여쓰기
    ...
    if(replyPagingCreator.replyList[i].lvl > 1){//답글인 경우, 앞에 아이콘 표시
        ...
        str   +=                     replyPagingCreator.replyList[i].mid 
            + '                </strong>'
            + '                <span>&nbsp;</span>'
    ...
    if(replyPagingCreator.replyList[i].lvl > 1){ //답글인 경우, 답글을 표시
        ...
        str   +='            </span>'
            ...
            + '               data-rno=' + replyPagingCreator.replyList[i].rno
            + '               data-mid=' + replyPagingCreator.replyList[i].mid
...
$(".chat").on("click", ".commentLi div div .btnModCmt", function(){
	...
	var midVal = $(this).siblings("p").data("mid");
	var pageNum = frmCmtPagingValue.find("input[name ='pageNum']").val();
...
/*  //댓글 목록 데이터 요청/받기 Ajax 테스트   
 ...
 //댓글등록  테스트
/* myCommentClsr.registerCmt(
    {bno:bnoValue, rcontent:"JS-클로저-댓글입력테스트입니다.", mid: "user7"} , //1번째: comment인자값 
...
//답글등록 테스트
//myCommentClsr.registerReply(
//    {bno: bnoValue, prno: 1, rcontent: "JS-클로저-댓글의 답글입력테스트입니다.", mid: "user10"}, 
...
myCommentClsr.removeCmtReply(
    {bno : bnoValue, rno : 22, mid: "user10"}, //댓글-답글 입력 테스트 시, SQL*Developer 에서 확인한 rno를 사용
...
<script>
	...
	function getCmtList(page){
		...
		for (var i = 0, len = bCommentPagingCreator.CmtList.length; i < len; i++){
			str +='<li class="left clearfix commentLi data-bno="' + bnoValue
			+ '"	data-cno"'+bCommentPagingCreator.CmtList[i].rno+"'>'
			+'        <div>';
	//댓글에 대한 답글 들여쓰기
		...
		if(bCommentPagingCreator.CmtList[i].lvl > 1){
			...
			str +='         <span>'
				...
				+ '            data-rno=' + bCommentPagingCreator.CmtList[i].rno
